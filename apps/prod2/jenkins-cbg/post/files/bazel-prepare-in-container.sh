...
# Requirements:
# 1. (optional) mount downward api volume to /etc/containerinfo:
#    volumes:
#    - name: containerinfo
#      downwardAPI:
#        items:
...
# 2. (optional) need a PV or PVC mounted at /share/.cache/bazel-repository-cache with `ReadWriteMany`` mode.
